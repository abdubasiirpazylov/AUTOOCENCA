        ...
        with st.spinner("Нейросеть Google Tesseract читает документы..."):
            try:
                ...
                for i, text in enumerate(upper_lines):
                    ...
                    # 1. ФИО Владельца
                    if (
                            "ВЛАДЕЛЕЦ" in text or "ЭЭСИ" in text or "OWNER" in text) and "АДРЕС" not in text and "ADDRESS" not in text:
                        if i + 1 < len(lines):
                ...
                if brand or model:
                    st.session_state.auto_model = f"{brand} {model}".strip()

                # =========================================================
                # УМНЫЙ И БРОНЕБОЙНЫЙ ПОИСК ГОСНОМЕРА
                # =========================================================
                # Шаг 1: Ищем только современный формат KG (например, 01KG339ASF) по сплошной строке
                ...
                if reg_match_kg:
                    ...
                    # Шаг 2: Если нового формата нет, ищем старый формат (например, B1234AB)
                    # Но строго с границами слов (\b), чтобы буквы из слов не прилипали к цифрам!
